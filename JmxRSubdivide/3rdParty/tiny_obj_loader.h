...
struct vertex_Para_Index {
  int v_idx, vt_idx, vn_idx;
  vertex_Para_Index() : v_idx(-1), vt_idx(-1), vn_idx(-1) {}
  explicit vertex_Para_Index(int idx) : v_idx(idx), vt_idx(idx), vn_idx(idx) {}
  vertex_Para_Index(int vidx, int vtidx, int vnidx)
      : v_idx(vidx), vt_idx(vtidx), vn_idx(vnidx) {}
};

struct tag_sizes {
  tag_sizes() : num_Para_Ints(0), num_floats(0), num_strings(0) {}
  int num_Para_Ints;
  int num_floats;
  int num_strings;
};
...
// for std::map
static inline bool operator<(const vertex_Para_Index &a, const vertex_Para_Index &b) {
  if (a.v_idx != b.v_idx)
...
static vertex_Para_Index parseTriple(const char *&token, int vsize, int vnsize,
                                int vtsize) {
  vertex_Para_Index vi(-1);
...
static unsigned int
updateVertex(std::map<vertex_Para_Index, unsigned int> &vertexCache,
             std::vector<float> &positions, std::vector<float> &normals,
             std::vector<float> &texcoords,
             const std::vector<float> &in_positions,
             const std::vector<float> &in_normals,
             const std::vector<float> &in_texcoords, const vertex_Para_Index &i) {
  const std::map<vertex_Para_Index, unsigned int>::iterator it = vertexCache.find(i);
...
static bool exportFaceGroupToShape(
    shape_t &shape, std::map<vertex_Para_Index, unsigned int> vertexCache,
    const std::vector<float> &in_positions,
    const std::vector<float> &in_normals,
    const std::vector<float> &in_texcoords,
    const std::vector<std::vector<vertex_Para_Index> > &faceGroup,
  ...
  for (size_t i = 0; i < faceGroup.size(); i++) {
    const std::vector<vertex_Para_Index> &face = faceGroup[i];

    vertex_Para_Index i0 = face[0];
    vertex_Para_Index i1(-1);
    vertex_Para_Index i2 = face[1];
...
bool LoadObj(std::vector<shape_t> &shapes,       // [output]
  ...
  std::vector<std::vector<vertex_Para_Index> > faceGroup;
  std::string name;

  // material
  std::map<std::string, int> material_map;
  std::map<vertex_Para_Index, unsigned int> vertexCache;
  int material = -1;
  ...
  while (inStream.peek() != -1) {
    ...
    if (token[0] == 'f' && IS_SPACE((token[1]))) {
      ...
      std::vector<vertex_Para_Index> face;
      face.reserve(3);

      while (!IS_NEW_LINE(token[0])) {
        vertex_Para_Index vi = parseTriple(token, static_cast<int>(v.size() / 3),
      ...
      faceGroup.push_back(std::vector<vertex_Para_Index>());
    ...
    if (token[0] == 't' && IS_SPACE(token[1])) {
      ...
      tag.intValues.resize(static_cast<size_t>(ts.num_Para_Ints));

      for (size_t i = 0; i < static_cast<size_t>(ts.num_Para_Ints); ++i) {
